...
def get_ds(config):
    ...
    filtered_sorted_train_ds = [k for k in filtered_sorted_train_ds if len(k["translation"][config["lang_src"]]) < 150]
    # filtered_sorted_train_ds = [k for k in sorted_train_ds if len(k["translation"][config["lang_tgt"]]) < 150]

    print(len(filtered_sorted_train_ds))
    ...
    # collator = DataCollatorWithPadding(tokenizer=tokenizer_src, max_length=150)
    train_dataloader = DataLoader(train_ds, batch_size=config['batch_size'], shuffle=True,collate_fn=train_ds.collate_samples)
...
#         for item in batch:
#             encoder_inputs.append(b["encoder_inputs"][:encoder_input_max])
#             decoder_inputs.append(b["decoder_inputs"][:decoder_input_max])
#             encoder_mask.append(b["encoder_mask"][0,0:encoder_input_max]).unqueeze(0).unqueeze(0)
#             decoder_mask.append(b["decoder_mask"][0,:decoder_input_max,:decoder_input_max]).unqueeze(0)
#             label.append(b["label"][:decoder_input_max])
#             src_text.append(b["src_text"])
#             tgt_text.append(b["tgt_text"])


#         return {"encoder_inputs": torch.vstack(encoder_inputs),
...
#                 "src_text": torch.vstack(src_text),"tgt_text": torch.vstack(tgt_text)
        ...
        for batch in batch_iterator:
          ...
          batch_iterator.set_postfix(
                {"loss_acc":f"{torch.mean(torch.stack(loss_acc)).item():6.3f}",
                 ...
                 "sql_1:" : f"{batch['encoder_input'].shape[1]}"})
